export function genCircomAllstr(graph_json, template_name, reveals, regex) {
  ...
  //multior
  declarations.push(`template MultiOR(n) {`);
  declarations.push(`\tsignal input in[n];`);
  declarations.push(`\tsignal output out;\n`);

  declarations.push(`\tsignal sums[n];`);
  declarations.push(`\tsums[0] <== in[0];`);
  declarations.push(`\tfor (var i = 1; i < n; i++) {`);
  declarations.push(`\t\tsums[i] <== sums[i-1] + in[i];`);
  declarations.push(`\t}\n`);

  declarations.push(`\tcomponent is_zero = IsZero();`);
  declarations.push(`\tis_zero.in <== sums[n-1];`);
  declarations.push(`\tout <== 1 - is_zero.out;`);
  declarations.push(`}\n`);
  //
  ...
  for (let i = 0; i < reveals.length; i++) {
    ...
    reveal_lines.push("\tassert(out == 1);");
    reveal_lines.push("\t}");
  }
  lines = lines.concat(reveal_lines);
  lines.push("}");
  lines.push("component main = Test(100);");
